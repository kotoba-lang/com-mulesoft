(ns mulesoft.main
  "Kotodama WASM entrypoint for the Mulesoft clean-room actor (L4) — Clojure port.
  ...
  facts (`mulesoft.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mulesoft")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"mulesoft.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mulesoft-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mulesoft-compat/ready)
